...
const corsHeaders = {
  'Access-Control-Allow-Origin': 'https://saudebucaljaperi.com.br',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: senhaValida, error: erroSenha } = await admin.rpc('verificar_senha_admin', { p_senha: senha });
    ...
    const { data: urlAssinada, error: erroUrl } = await admin.storage
      ...
      .createSignedUrl(solicitacao.documento_path, 300, { download: solicitacao.documento_nome || 'documento-justificativa' });
